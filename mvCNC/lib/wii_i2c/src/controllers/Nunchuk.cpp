...
namespace NintendoExtensionCtrl {
  ...
  ExtensionType NunchukBase::getExpectedType() const { return ExtensionType::Nunchuk; }

  uint8_t NunchukBase::joyX() const { return getControlData(Maps::JoyX); }

  uint8_t NunchukBase::joyY() const { return getControlData(Maps::JoyY); }
  ...
  boolean NunchukBase::buttonC() const { return getControlBit(Maps::ButtonC); }

  boolean NunchukBase::buttonZ() const { return getControlBit(Maps::ButtonZ); }

  float NunchukBase::rollAngle() const { return atan2((float)accelX() - 511.0, (float)accelZ() - 511.0) * 180.0 / PI; }
  ...
  void NunchukBase::printDebug(Print &output) const {
    ...
    snprintf(buffer, sizeof(buffer), "Joy:(%3u, %3u) | Accel XYZ:(%4u, %4u, %4u) | Buttons: %c%c", joyX(), joyY(),
             accelX(), accelY(), accelZ(), cPrint, zPrint);
