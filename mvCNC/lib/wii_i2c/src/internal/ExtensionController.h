...
namespace NintendoExtensionCtrl {
  ...
  public:
    struct ExtensionData {
      ...
      ExtensionData(NXC_I2C_TYPE &i2cbus) : i2c(i2cbus) {}
  ...
  public:
    ...
    inline boolean requestData(uint8_t ptr, size_t size, uint8_t *dataOut) const {
      return requestData(data.i2c, ptr, size, dataOut);
    }
    inline boolean requestControlData(size_t size, uint8_t *controlData) const {
      return requestControlData(data.i2c, size, controlData);
    }
  ...
  protected:
    ...
    uint8_t getControlData(const ByteMap map) const { return (data.controlData[map.index] & map.mask) >> map.offset; }

    template <size_t size> uint8_t getControlData(const ByteMap (&map)[size]) const {
  ...
  class ExtensionClassBundle : protected ExtensionDataWrapper,
                               public ControllerSource {
  public:
    ExtensionClassBundle(NXC_I2C_TYPE &i2cBus = NXC_I2C_DEFAULT) :
      ExtensionDataWrapper({i2cBus}), ControllerSource(dataInstance) {}
  };
  ...
  private:
    ...
    template <typename T> friend class ExtensionPortVariant;
  };
  ...
  template <class ControllerMap> class ExtensionPortVariant : public ControllerMap {
  public:
    ExtensionPortVariant(ExtensionPort &port) : ControllerMap(port.dataInstance), node(port.list, *this) {}
  ...
  template <class ControllerMap> class BuildControllerClass : public ExtensionClassBundle<ControllerMap> {
  public:
