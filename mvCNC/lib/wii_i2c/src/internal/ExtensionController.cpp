...
namespace NintendoExtensionCtrl {
  ...
  ExtensionController::ExtensionController(ExtensionData &dataRef) : data(dataRef) {}
  ...
  ExtensionType ExtensionController::getExpectedType() const { return ExtensionType::AnyController; }

  ExtensionType ExtensionController::getControllerType() const { return data.connectedType; }
  ...
  uint8_t ExtensionController::getControlData(uint8_t controlIndex) const { return data.controlData[controlIndex]; }

  void ExtensionController::setControlData(uint8_t index, uint8_t val) { data.controlData[index] = val; }

  ExtensionController::ExtensionData &ExtensionController::getExtensionData() const { return data; }

  size_t ExtensionController::getRequestSize() const { return data.requestSize; }

  void ExtensionController::setRequestSize(size_t r) {
    if (r >= MinRequestSize && r <= MaxRequestSize) { data.requestSize = (uint8_t)r; }
  }

  NXC_I2C_TYPE &ExtensionController::i2c() const { return data.i2c; }

  void ExtensionController::printDebug(Print &output) const { printDebugRaw(output); }
  ...
  void ExtensionController::printDebugRaw(Print &output) const { printDebugRaw(HEX, output); }
