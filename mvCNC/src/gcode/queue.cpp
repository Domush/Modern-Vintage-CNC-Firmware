...
void GCodeQueue::get_serial_commands() {
  ...
  // send "wait" to indicate mvCNC is still waiting.
  #if TIMEOUT_PREVENTION_DELAY > 0
      ...
      if (ISEOL(serial_char)) {
        ...
        if (IsStopped()) {
          ...
          if (command[0] == 'M') switch (command[3]) {
            case '8': if (command[2] == '0' && command[1] == '1') { wait_for_heatup = false; TERN_(HAS_MVCNCUI_MENU, wait_for_user = false); } break;
      ...
      if (is_eol || card_eof) {
        ...
        if (!process_line_done(sd_input_state, command.buffer, sd_count)) {
          ...
          TERN_(GCODE_REPEAT_MARKERS, repeat.early_parse_M808(command.buffer));
            ...
            // Otherwise the M125 handler needs to know SD job is active
