...
// All displays share the mvCNCUI class
#include "mvcncui.h"
mvCNCUI ui;
  ...
  bool mvCNCUI::wait_for_move; // = false
#endif
...
constexpr uint8_t epps = ENCODER_PULSES_PER_STEP;
  ...
  mvCNCUI::progress_t mvCNCUI::progress_override; // = 0
  #if ENABLED(USE_M73_REMAINING_TIME)
    uint32_t mvCNCUI::remaining_time;
  #endif
#endif

#if HAS_MULTI_LANGUAGE
  uint8_t mvCNCUI::language; // Initialized by settings.load()
...
#if HAS_LCD_CONTRAST
  uint8_t mvCNCUI::contrast = LCD_CONTRAST_DEFAULT; // Initialized by settings.load()
  void mvCNCUI::set_contrast(const uint8_t value) {
  ...
  uint8_t mvCNCUI::brightness = LCD_BRIGHTNESS_DEFAULT;
  bool mvCNCUI::backlight = true;

  void mvCNCUI::set_brightness(const uint8_t value) {
    backlight = !!value;
  ...
  bool mvCNCUI::buzzer_enabled = true;
#endif
  ...
  #endif
  void mvCNCUI::buzz(const long duration, const uint16_t freq) {
...
// Encoder Handling
  ...
  uint32_t mvCNCUI::encoderPosition;
...
void mvCNCUI::init() {
  ...
  #if HAS_ENCODER_ACTION
    volatile uint8_t mvCNCUI::buttons;
    #if HAS_SLOW_BUTTONS
      volatile uint8_t mvCNCUI::slow_buttons;
    #endif
    #if HAS_TOUCH_BUTTONS
      #include "touch/touch_buttons.h"
      bool mvCNCUI::on_edit_screen = false;
    #endif
  #endif

  #if SCREENS_CAN_TIME_OUT
    bool mvCNCUI::defer_return_to_status;
    millis_t mvCNCUI::return_to_status_ms = 0;
  #endif

  uint8_t mvCNCUI::lcd_status_update_delay = 1; // First update one loop delayed

  #if BOTH(FILAMENT_LCD_DISPLAY, SDSUPPORT)
    millis_t mvCNCUI::next_filament_display; // = 0
  #endif
  ...
  millis_t mvCNCUI::next_button_update_ms; // = 0
    ...
    bool mvCNCUI::old_is_cutting;
      ...
      uint8_t mvCNCUI::filename_scroll_pos, mvCNCUI::filename_scroll_max;
    #endif

    const char * mvCNCUI::scrolled_filename(CardReader &theCard, const uint8_t maxlen, uint8_t hash, const bool doScroll) {
    ...
    bool mvCNCUI::screen_changed;

    #if ENABLED(ENCODER_RATE_MULTIPLIER)
      bool mvCNCUI::encoderRateMultiplierEnabled;
      millis_t mvCNCUI::lastEncoderMovementMillis = 0;
      void mvCNCUI::enable_encoder_multiplier(const bool onoff) {
      ...
      int8_t mvCNCUI::encoderDirection = ENCODERBASE;
    #endif
      ...
      void mvCNCUI::draw_select_screen_prompt(PGM_P const pref, const char * const string/*=nullptr*/, PGM_P const suff/*=nullptr*/) {
   ...
   */
    ...
    millis_t mvCNCUI::progress_bar_ms; // = 0
    #if PROGRESS_MSG_EXPIRE > 0
      millis_t mvCNCUI::expire_status_ms; // = 0
    #endif
  #endif
  ...
  LCDViewAction mvCNCUI::lcdDrawUpdate = LCDVIEW_CLEAR_CALL_REDRAW;
  ...
  bool mvCNCUI::has_status() { return (status_message[0] != '\0'); }

  void mvCNCUI::set_status(const char * const cstr, const bool persist) {
  ...
  void mvCNCUI::reset_status(const bool no_welcome) {
  ...
  void mvCNCUI::set_status(FSTR_P const fstr, int8_t level) {
  ...
  void mvCNCUI::status_printf(const uint8_t level, FSTR_P const fmt, ...) {
    if (level < alert_level) return;
    alert_level = level;
    va_list args;
  ...
  void mvCNCUI::finish_status(const bool persist) {
    ...
    void mvCNCUI::advance_status_scroll() {
    ...
    char* mvCNCUI::status_and_len(uint8_t &len) {
  ...
  void mvCNCUI::resume_print() {
    ...
    //
    void mvCNCUI::screen_click(const uint8_t row, const uint8_t col, const uint8_t, const uint8_t) {
      const millis_t now = millis();
  ...
  //
  void mvCNCUI::set_status(const char * const cstr, const bool) {
    TERN(HOST_PROMPT_SUPPORT, hostui.notify(cstr), UNUSED(cstr));
  }
  void mvCNCUI::set_status(FSTR_P const fstr, const int8_t) {
    TERN(HOST_PROMPT_SUPPORT, hostui.notify(fstr), UNUSED(fstr));
  }
  void mvCNCUI::status_printf(const uint8_t, FSTR_P const fstr, ...) {
    TERN(HOST_PROMPT_SUPPORT, hostui.notify(fstr), UNUSED(fstr));
  }
  ...
  void mvCNCUI::media_changed(const uint8_t old_status, const uint8_t status) {
  ...
  void mvCNCUI::pause_show_message(
    ...
    void mvCNCUI::load_settings() {
      const bool good = settings.load();
      completion_feedback(good);
    }
    void mvCNCUI::store_settings() {
