...
//
  ...
  void menu_show_mvcnc_bootscreen() {
    if (ui.use_click()) { ui.goto_previous_screen_no_defer(); }
    ui.draw_mvcnc_bootscreen();
  }

  #if ENABLED(SHOW_CUSTOM_BOOTSCREEN)
    void menu_show_custom_bootscreen() {
      if (ui.use_click()) { ui.goto_screen(menu_show_mvcnc_bootscreen); }
  ...
  void menu_info_cnc() {
    if (ui.use_click()) return ui.go_back();
    START_SCREEN();
    STATIC_ITEM(MSG_mvCNC, SS_DEFAULT|SS_INVERT);              // mvCNC
    ...
    PSTRING_ITEM(MSG_INFO_ATC_TOOLS, STRINGIFY(ATC_TOOLS), SS_CENTER); // Extruders: 2
    #if HAS_LEVELING
      STATIC_ITEM(
...
void menu_info() {
  ...
  BACK_ITEM(MSG_MAIN);
  #if ENABLED(LCD_PRINTER_INFO_IS_BOOTSCREEN)
    SUBMENU(MSG_INFO_PRINTER_MENU, TERN(SHOW_CUSTOM_BOOTSCREEN, menu_show_custom_bootscreen, menu_show_mvcnc_bootscreen));
  #else
