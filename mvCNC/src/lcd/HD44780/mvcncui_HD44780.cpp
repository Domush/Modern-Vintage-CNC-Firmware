...
void mvCNCUI::set_custom_characters(const HD44780CharSet screen_charset/*=CHARSET_INFO*/) {
...
bool mvCNCUI::detected() {
  return TERN1(DETECT_I2C_LCD_DEVICE, lcd.LcdDetected() == 1);
}

#if HAS_SLOW_BUTTONS
  uint8_t mvCNCUI::read_slow_buttons() {
...
void mvCNCUI::clear_lcd() { lcd.clear(); }
  ...
  static void logo_lines(PGM_P const extra) {
    ...
    lcd_put_u8str(indent, 1, F("|mvCNC|"));  lcd_put_u8str_P(extra);
  ...
  void mvCNCUI::show_bootscreen() {
    ...
    //
    // Show the mvCNC logo with splash line 1
    //
    if (LCD_EXTRA_SPACE >= utf8_strlen(SHORT_BUILD_VERSION) + 1) {
      ...
      //
      // Show the mvCNC logo and short build version
  ...
  void mvCNCUI::bootscreen_completion(const millis_t) {
    lcd.clear();
    safe_delay(100);
...
FORCE_INLINE void _draw_bed_status(const bool blink) {
  ...
  void mvCNCUI::draw_progress_bar(const uint8_t percent) {
...
void mvCNCUI::draw_hotend_status(const uint8_t row, const uint8_t atc_tool) {
  ...
  void MenuItem_confirm::draw_select_screen(PGM_P const yes, PGM_P const no, const bool yesno, PGM_P const pref, const char * const string/*=nullptr*/, PGM_P const suff/*=nullptr*/) {
    ...
    void mvCNCUI::update_indicators() {
      ...
      if (TERN0(TOOL_CHANGE_SUPPORT, fanManager.degTargetHotend(1) > 0)) leds |= LED_C;
