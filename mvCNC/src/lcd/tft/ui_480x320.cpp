...
void mvCNCUI::tft_idle() {
  ...
  void mvCNCUI::show_bootscreen() {
    ...
    #else
      #define BOOT_LOGO_W TFT_WIDTH   // mvCNCLogo480x320x16
...
void mvCNCUI::draw_status_screen() {
  ...
  tft_string.add('%');
  tft.add_text(36, 1, color, tft_string);
  TERN_(TOUCH_SCREEN, touch.add_control(FLOWRATE, 284, 176, 100, 32, active_tool));

  #if ENABLED(TOUCH_SCREEN)
  add_control(404, y, menu_main, imgSettings);
  TERN_(SDSUPPORT, add_control(12, y, menu_media, imgSD, !jobIsActive(), COLOR_CONTROL_ENABLED,
...
static void drawBtn(int x, int y, const char *label, intptr_t data, mvCNCImage img, uint16_t bgColor, bool enabled = true) {
