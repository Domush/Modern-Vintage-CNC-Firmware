...
void Touch::touch(touch_control_t *control) {
  switch (control->type) {
    ...
    case FAN:
      ...
      MenuItem_percent::action((const char *)GET_TEXT_F(MSG_FIRST_FAN_SPEED), &fan_speed, 0, 255, []{ fanManager.fanSpeedSet(fan, fan_speed); });
      break;
    case FEEDRATE:
      ui.clear_lcd();
...
bool mvCNCUI::touch_pressed() {
  return touch.is_clicked();
}

void add_control(uint16_t x, uint16_t y, TouchControlType control_type, intptr_t data, mvCNCImage image, bool is_enabled, uint16_t color_enabled, uint16_t color_disabled) {
