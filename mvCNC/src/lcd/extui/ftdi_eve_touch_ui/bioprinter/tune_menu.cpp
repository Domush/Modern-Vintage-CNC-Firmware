...
void TuneMenu::onRedraw(draw_mode_t what) {
  ...
  if (what & FOREGROUND) {
    ...
    cmd.colors(normal_btn)
       .font(font_medium)
       .enabled( jobRunning()).tag(2).button(BTN_POS(1,2), BTN_SIZE(2,1), GET_TEXT_F(MSG_PRINT_SPEED))
       ...
       .enabled(!jobRunning()).tag(5).button(BTN_POS(1,5), BTN_SIZE(2,1), GET_TEXT_F(MSG_MOVE_TO_HOME))
       .enabled(!jobRunning()).tag(6).button(BTN_POS(1,6), BTN_SIZE(2,1), GET_TEXT_F(MSG_RAISE_PLUNGER))
       .enabled(!jobRunning()).tag(7).button(BTN_POS(1,7), BTN_SIZE(2,1), GET_TEXT_F(MSG_RELEASE_XY_AXIS))
