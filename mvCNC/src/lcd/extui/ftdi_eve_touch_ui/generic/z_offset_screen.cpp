...
void ZOffsetScreen::onRedraw(draw_mode_t what) {
  ...
  w.increments();
  w.button(2, GET_TEXT_F(MSG_PROBE_WIZARD), !jobRunning() && !wizardRunning());
}
...
void ZOffsetScreen::runWizard() {
  ...
  AlertDialogBox::show(F("After the cnc finishes homing, adjust the Z Offset so that a sheet of paper can pass between the nozzle and bed with slight resistance."));
}
