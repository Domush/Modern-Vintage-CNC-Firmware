...
void ST7920_Lite_Status_Screen::draw_position(const xyze_pos_t &pos, const bool position_trusted) {
  ...
  if (TERN0(LCD_SHOW_E_TOTAL, jobIsActive())) {
    ...
    sprintf_P(tmp, PSTR("E%-7ld%cm "), uint32_t(_MAX(e_move_accumulator, 0.0f)) / escale,
...
void ST7920_Lite_Status_Screen::update_indicators(const bool forceUpdate) {
  if (forceUpdate || indicators_changed()) {
    ...
    #endif
    draw_fan_speed(fanManager.pwmToPercent(spd));
...
void mvCNCUI::lcd_in_status(const bool inStatus) {
