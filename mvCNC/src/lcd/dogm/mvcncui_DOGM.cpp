 ...
 * by STB for ErikZalm/mvCNC. Common LCD 128x64 pixel graphic displays.
 *
...
#if HAS_LCD_CONTRAST
  void mvCNCUI::_set_contrast() { u8g.setContrast(contrast); }
#endif

void mvCNCUI::set_font(const mvCNCFont font_nr) {
...
bool mvCNCUI::detected() { return true; }
    ...
    void mvCNCUI::draw_custom_bootscreen(const uint8_t frame/*=0*/) {
    ...
    void mvCNCUI::show_custom_bootscreen() {
  ...
  // Draw the static mvCNC bootscreen from a u8g loop
  ...
  void mvCNCUI::draw_mvcnc_bootscreen(const bool line2/*=false*/) {
    ...
    auto draw_bootscreen_bmp = [&](const uint8_t *bitmap) {
      ...
      LOOP_L_N(f, COUNT(mvcnc_bootscreen_animation)) {
        draw_bootscreen_bmp((uint8_t*)pgm_read_ptr(&mvcnc_bootscreen_animation[f]));
        if (frame_time) safe_delay(frame_time);
      }
    #endif
  }

  // Show the mvCNC bootscreen, with the u8g loop and delays
  void mvCNCUI::show_mvcnc_bootscreen() {
    for (uint8_t q = bootscreen_pages; q--;) {
      draw_mvcnc_bootscreen(q == 0);
...
void mvCNCUI::update_language_font() {
...
void mvCNCUI::clear_lcd() { } // Automatically cleared by Picture Loop
