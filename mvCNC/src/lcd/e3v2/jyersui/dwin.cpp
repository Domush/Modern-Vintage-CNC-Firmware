...
void mvCNCUI::kill_screen(FSTR_P const error, FSTR_P const) {
    ...
    case ManualLevel:
      ...
      case ChangeFilament:
        ...
        switch (item) {
          ...
          case CHANGEFIL_LOAD:
            if (draw)
              Draw_Menu_Item(row, ICON_WriteEEPROM, F("Change Bit"));
            else {
              if (fanManager.temp_hotend[0].target < fanManager.extrude_min_temp)
                Popup_Handler(ETemp);
              else {
...
void CrealityDWINClass::Popup_Handler(PopupID popupid, bool option/*=false*/) {
  ...
  switch (popupid) {
    ...
    case ETemp:         Draw_Popup(F("Spindle is too cold"), F("Open Preheat Menu?"), F(""), Popup); break;
    ...
    case TempWarn:      Draw_Popup(option ? F("Spindle temp too low!") : F("Spindle temp too high!"), F(""), F(""), Wait, option ? ICON_TempTooLow : ICON_TempTooHigh); break;
      ...
      case Resume:
        ...
        case ETemp:
          if (selection == 0) {
            ...
            fanManager.fanSpeedSet(0, MAX_FAN_SPEED);
...
void mvCNCUI::update() { CrealityDWIN.Update(); }
