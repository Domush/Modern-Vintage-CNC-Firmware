...
FORCE_INLINE void _draw_axis_value(const AxisEnum axis, const char *value, const bool blink, const uint16_t x, const uint16_t y) {
    ...
    if (blink)
      ...
      dwin_string.add(TERN1(DWIN_mvCNCUI_PORTRAIT, axis == Z_AXIS) ? PSTR("       ") : PSTR("    "));
    else
      dwin_string.add(value);

    // For E_TOTAL there may be some characters to cover up
    if (BOTH(DWIN_mvCNCUI_PORTRAIT, LCD_SHOW_E_TOTAL) && axis == X_AXIS)
      dwin_string.add("   ");
...
/**
 * Draw the mvCNCUI Status Screen for Ender 3 V2
 */
void mvCNCUI::draw_status_screen() {
  ...
  if (!ui.did_first_redraw) {
    ...
    DWIN_ICON_Show(ICON, ICON_LOGO_mvCNC,
      #if ENABLED(DWIN_mvCNCUI_PORTRAIT)
  ...
  if (show_e_total) {
    TERN_(LCD_SHOW_E_TOTAL, _draw_e_value(e_move_accumulator, TERN(DWIN_mvCNCUI_PORTRAIT, 6, 75), cpy));
  }
  else {
                      _draw_axis_value(X_AXIS, ftostr4sign(lpos.x), blink, TERN(DWIN_mvCNCUI_PORTRAIT,  6,  75), cpy);
    TERN_(HAS_Y_AXIS, _draw_axis_value(Y_AXIS, ftostr4sign(lpos.y), blink, TERN(DWIN_mvCNCUI_PORTRAIT, 95, 184), cpy));
  }
  TERN_(HAS_Z_AXIS, _draw_axis_value(Z_AXIS, ftostr52sp(lpos.z), blink, TERN(DWIN_mvCNCUI_PORTRAIT, 165, 300), cpy));
  ...
  constexpr int16_t pb_margin = 5,
                    pb_left = pb_margin + TERN(DWIN_mvCNCUI_PORTRAIT, 0, 90),
                    pb_height = TERN(DWIN_mvCNCUI_PORTRAIT, 60, 20),
                    pb_right = LCD_PIXEL_WIDTH - pb_margin,
                    pb_bottom = TERN(DWIN_mvCNCUI_PORTRAIT, 410, 220),
